...
foreach (array_filter(array_map('trim', explode("\n", $output))) as $string) {
  ...
  if ($tag) {
    $version = preg_replace('/^[78]\.x-/', '', $tag);
    $require[] = "drupal/$project_name:$version";
  }
  else {
    $branch = trim(shell_exec('git branch -a --contains HEAD 2>/dev/null | grep -v "remotes/origin/HEAD" | grep "remotes/origin/" | head -n1 | sed -e "s/remotes\/origin\///"'));
    ...
    if (strpos($url, 'drupal.org') === FALSE) {
      ...
      $version = preg_replace('/^[78]\.x-/', '', $branch) . '-dev';
      $require[] = "drupal/$project_name:$version#$hash";
    }
  }
}
$require = 'composer require ' . implode(' ', $require);
